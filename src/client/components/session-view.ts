...
export class SessionView extends LitElement {
  ...
  // Mobile header auto-hide scroll tracking
  private mobileScrollStartY = 0;
  private mobileScrollLastY = 0;
  ...
  private async fetchTmuxStatus(tmuxSessionName: string) {
    try {
      const response = await fetch(
        `/multiplexer/tmux/sessions/${encodeURIComponent(tmuxSessionName)}/windows`,
        {
          headers: authClient.getAuthHeader(),
        }
      );
      ...
      this.tmuxStatus = windows
        .map(
          (w) => `${w.index}:${w.name}${w.active ? '*' : ''}${w.panes > 1 ? ` (${w.panes}P)` : ''}`
        )
        .join(' | ');
      this.requestUpdate();
    } catch {
  ...
  private handleMobileTouchStart = (e: TouchEvent) => {
    const touch = e.touches[0];
    this.mobileScrollStartY = touch.clientY;
    this.mobileScrollLastY = touch.clientY;
  };

  private handleMobileTouchMove = (e: TouchEvent) => {
    const touch = e.touches[0];
    const deltaY = touch.clientY - this.mobileScrollLastY;
    const totalDelta = touch.clientY - this.mobileScrollStartY;
    this.mobileScrollLastY = touch.clientY;

    const uiState = this.uiStateManager.getState();
    if (!uiState.isMobile) return;

    // Threshold: 30px of scrolling before showing/hiding
    if (Math.abs(totalDelta) < 30) return;

    if (deltaY < -5 && !uiState.mobileHeaderHidden) {
      // Scrolling down (finger moving up) -> hide header
      this.uiStateManager.setMobileHeaderHidden(true);
    } else if (deltaY > 5 && uiState.mobileHeaderHidden) {
      // Scrolling up (finger moving down) -> show header
      this.uiStateManager.setMobileHeaderHidden(false);
    }
  };

  private handleMobileTouchEnd = () => {
    this.mobileScrollStartY = 0;
    this.mobileScrollLastY = 0;
  };
    ...
    return html`
      <style>
        ...
        @media (max-width: 768px) {
          ...
          .session-header-area {
            /* Compact mobile header: 44px + auto-hide support */
            flex-shrink: 0 !important;
            position: relative !important;
            z-index: 50 !important;
            background-color: rgb(var(--color-bg-secondary)) !important;
            /* GPU-accelerated for smooth hide/show */
        ...
        /* Hide scrollbars on mobile special keys */
        #mobile-special-keys .overflow-x-auto::-webkit-scrollbar {
          display: none;
        }
        #mobile-special-keys .overflow-x-auto {
      ...
      <div class="bg-bg-secondary" style="padding-top: env(safe-area-inset-top);">
        ...
        <div class="session-header-area">
          <session-header
            ...
            .mobileHeaderHidden=${uiState.mobileHeaderHidden}
        ...
        <div
          ...
          @touchstart=${uiState.isMobile ? this.handleMobileTouchStart : undefined}
          @touchmove=${uiState.isMobile ? this.handleMobileTouchMove : undefined}
          @touchend=${uiState.isMobile ? this.handleMobileTouchEnd : undefined}
        >
          ${
            this.loadingAnimationManager.isLoading()
              ? html`
        ...
        ${
          ...
          <mobile-action-bar
            .visible=${true}
            .session=${this.session}
            .keyboardVisible=${uiState.keyboardHeight > 0 || uiState.showQuickKeys}
      ...
      <!-- Terminal Quick Keys (desktop only - mobile uses mobile-action-bar special keys) -->
      <terminal-quick-keys
        ...
        .visible=${!uiState.isMobile && uiState.showQuickKeys && !uiState.chatMode}
