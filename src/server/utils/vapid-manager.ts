 ...
 * used for authenticating push notifications sent from the VibeTerm server to web clients.
 ...
 * - Persistent storage in ~/.vibeterm/vapid/keys.json
 *
 * @example
 * ```typescript
...
export class VapidManager {
  ...
  constructor(vapidDir?: string) {
    this.vapidDir = vapidDir || path.join(os.homedir(), '.vibeterm/vapid');
  ...
  async initialize(options: {
    ...
    if (publicKey && privateKey) {
      ...
      this.config = {
        ...
        contactEmail: contactEmail || 'noreply@vibeterm.local',
        enabled: true,
      };
  ...
  async rotateKeys(contactEmail?: string): Promise<VapidKeyPair> {
    ...
    this.config = {
      keyPair: newKeys,
      contactEmail: contactEmail || this.config?.contactEmail || 'noreply@vibeterm.local',
      enabled: true,
    };
