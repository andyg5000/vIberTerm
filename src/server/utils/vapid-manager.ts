 ...
 * used for authenticating push notifications sent from the VibeTmux server to web clients.
 ...
 * - Persistent storage in ~/.vibetmux/vapid/keys.json
 *
 * @example
 * ```typescript
...
export class VapidManager {
  ...
  constructor(vapidDir?: string) {
    this.vapidDir = vapidDir || path.join(os.homedir(), '.vibetmux/vapid');
  ...
  async initialize(options: {
    ...
    if (publicKey && privateKey) {
      ...
      this.config = {
        ...
        contactEmail: contactEmail || 'noreply@vibetmux.local',
        enabled: true,
      };
  ...
  async rotateKeys(contactEmail?: string): Promise<VapidKeyPair> {
    ...
    this.config = {
      keyPair: newKeys,
      contactEmail: contactEmail || this.config?.contactEmail || 'noreply@vibetmux.local',
      enabled: true,
    };
