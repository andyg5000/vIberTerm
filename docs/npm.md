# VibeTmux NPM Package Distribution

This document explains the npm package build process, native module handling, and prebuild system for VibeTmux.

## Overview

VibeTmux is distributed as an npm package that includes:
...
- Command-line tools (`vibetmux` and `vt`)
...
│   ├── vibetmux          # Main server executable
...
VibeTmux requires two native modules:
...
- **Note**: While macOS uses different authentication mechanisms internally (OpenDirectory), VibeTmux attempts PAM authentication on both platforms as a fallback after SSH key authentication
...
### For End Users
1. **Install package**: `npm install -g vibetmux`
...
4. **Result**: Working VibeTmux installation without build tools
...
- If Mac app found at `/Applications/VibeTmux.app`, npm `vt` defers to it
...
- Use `vibetmux` or `npx vt` as alternatives
...
4. **Test the package contents**:
   ...
   tar -tzf vibetmux-*.tgz | grep authenticate-pam
...
**Detection**: VibeTmux's postinstall script will warn if this conflict is detected:
```
...
- **Control**: Custom modifications for VibeTmux's needs
...
**Published to npm**: Successfully published as both `vibetmux@beta` and `vibetmux@latest`
...
npm install -g vibetmux@beta

# Or install specific version
npm install -g vibetmux@1.0.0-beta.14.1
```
...
**Published to npm**: Successfully published as `vibetmux@beta`
...
1. **Version Synchronization**:
   - Must update version in both `web/package.json` and `mac/VibeTmux/version.xcconfig`
...
# Verify package contents
tar -tzf vibetmux-1.0.0-beta.11.tgz | head -50

# Test with Docker
docker build -t vibetmux-test .
docker run --rm vibetmux-test

# Test cross-platform
docker run --rm --platform linux/amd64 vibetmux-test
```
...
VibeTmux uses npm dist-tags to manage different release channels:
...
```bash
# View current tags
npm dist-tag ls vibetmux

# Set a version as latest
npm dist-tag add vibetmux@1.0.0-beta.13 latest

# Add a new tag
npm dist-tag add vibetmux@1.0.0-beta.14 next

# Remove a tag
npm dist-tag rm vibetmux next
```
...
npm install -g vibetmux@beta
npm install -g vibetmux@latest

# Install specific version
npm install -g vibetmux@1.0.0-beta.11.1
```
